...
(function($){
	$.addFlex = function(t,p)
	{
		...
		p = $.extend({
			...
			pagetext: 'Page',
			datetextrange: 'Showing {from} through {to}',
			datetextsingle: 'Showing {from}',
			datetextempty: 'Showing all time',
			range: '',
			outof: 'of',
			findtext: 'Find',
		...
		var g = {
			...
			prettydate: function(p) {
				...
				if (!p) { return; }
				var d = $.datepicker.parseDate('yy-mm-ddT00:00:00Z', p);
				var date = d.getDate();
				var month = d.getMonth();
				var year = d.getFullYear();
				return(m_names[month] + " " + date + ", " + year);
			},
			buildpager: function(){ //rebuild pager based on new properties
				...
				range = document.getElementById('pDateButton').value;
				if (!range) { range = ""; }
				range = range.split(" - ");
				if (range[0] && range[1]) {
					stat = p.datetextrange;
				}
				if (!range[0]) {
					stat = p.datetextempty;
				}
				if (range[0] && !range[1]) {
					stat = p.datetextsingle;
				}
				stat = stat.replace(/{from}/, this.prettydate(range[0]));
				stat = stat.replace(/{to}/, this.prettydate(range[1]));
				$('.pRangeStat',this.pDiv).html(stat);
			},
			populate: function () { //get latest data
				...
				var param = [
					...
					{ name : 'range', value : p.range}
				];
				if (p.params)
					{
						for (var pi = 0; pi < p.params.length; pi++) param[param.length] = p.params[pi];
					}
					$('input.limit').attr('value', p.rp);
					$.ajax({
						type: p.method,
						url: p.url,
						data: param,
		...
		$(g.bDiv).after(g.pDiv);
		var html = ' <div class="pGroup"><div class="pChangeRange pButton"><span id="pDateButton"></span></div></div> <div class="btnseparator"></div> <div class="pGroup"> <div class="pFirst pButton"><span></span></div><div class="pPrev pButton"><span></span></div> </div> <div class="btnseparator"></div> <div class="pGroup"><span class="pcontrol">'+p.pagetext+' <input type="text" size="4" value="1" /> '+p.outof+' <span> 1 </span></span></div> <div class="btnseparator"></div> <div class="pGroup"> <div class="pNext pButton"><span></span></div><div class="pLast pButton"><span></span></div> </div> <div class="btnseparator"></div> <div class="pGroup"> <div class="pReload pButton"><span></span></div> </div> <div class="btnseparator"></div> <div class="pGroup"><span class="pPageStat"></span></div>';
...
//				$('.pSearch',g.pDiv).click(function(){g.doSearch()});
				//add search box
	...
	$.fn.changeRange = function(data) {
		return this.each( function() {
			if (this.grid) this.grid.changeRange(data);
		});
	}
